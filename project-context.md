...
- Tests: **xUnit 2.9.2**, Test SDK 17.11.1, runner 2.8.2 — `dotnet test tests/PWRUHelper.Tests` (1337 tests, headless-safe, incl. STA render tests).
...
- Pipeline shape (**both chains built by `Services/TranslationChains.cs`**, E3.S7 — the composition sites in the code-behind never name `ProviderGates`, TP-START-02): read = `CachingTranslator( google-dict → google-gtx )`, write = `CachingTranslator( [deepl if key] → google-dict → google-gtx )`, rebuilt live on key save (`BuildWriteChain()`). The builders return the chain **already wrapped** in a `CachingTranslator` over the ONE shared `TranslationCacheStore` (`TranslationChains.Cache`, capacity 2000, persisted to `translation-cache.json`, E4.S4) — so the three chains share one cache, a key save keeps it, and no source outside `Services/` names a store or a decorator (scanned). `google-dict` is the DEFAULT first free tier; `edge` is absent from A.1 (ruling E3-d, U2 owner-blocked). **DeepL is absent from the read chain BY CONSTRUCTION (I8)** — its free plan is a one-time 1 M characters and the LIVE loop would drain it in days; pinned by `ChainCompositionTests.TP_CHN_14_*`. Read-chain providers carry `RequestPriority.Background` (§5.4's reserve) and read-once gets its own `Interactive` chain over the same gates — the priority is per INSTANCE because `ITranslator` may not grow a parameter (I1). `ChainTranslator` (E3.S3) replaced `FallbackTranslator`: tiers are tried in order, a tier whose **gate** is inside a block window is SKIPPED (no request, no delay), and when every tier was skipped the caller gets one `AllProvidersPaused` carrying the EARLIEST remembered window. The chain never calls `TryEnter` — `HttpProviderCore` is the one admission point — and it skips on `BlockedUntil > Now()`, never on `State == Open` (that state outlives its window on purpose; skipping on it pauses the provider forever).
- **Offline rung (E8, shipped v0.15.2).** Both chains end with the optional `BergamotTranslator`, appended last behind the one predicate `OfflineTierIsAvailable` (the `OfflineFallbackEnabled` setting **and** the store's installed answer — never the setting alone). Its files come only from the `offline-engine-v1` GitHub release and are verified — size then SHA-256 — against `OfflineModelManifest.Shipping`, which ships inside the exe: **never widen the download allowlist, and never edit that manifest without republishing the release**, because `BergamotEngine.Resolve` re-hashes `bergamot.dll` before it loads it and the native directory must hold **exactly one `.dll`** (an absolute-path load searches beside the file). The shipping RAM ceiling is **150 MiB while active** for the one `tiny` ru→en pair (ruling E8-a); A-1(c)'s 310 MiB was a second-pair budget and no second pair is in scope.
- Provider types are named after their endpoint, not after the app: `google-gtx` is `GoogleGtxTranslator` (`Services/GoogleGtxTranslator.cs`, E3.S6 — it was `TranslationService`, which named the whole feature after one free endpoint that is now the LAST free tier). `ITranslator` deliberately still lives in that file (I1). The GET-query chunker moved out with the rename to `Services/TextChunker.cs`, because E3.S4's `GoogleDictTranslator` needs the same byte budget (`TranslationPolicy.MaxQueryBytes`).
- The shared per-line loop is `Services/PerLineFallback.cs` (E3.S8). It latches on a REFUSAL only (`RateLimited`/`Blocked`): a timeout or an unparseable body fails its own line and the loop keeps going — widening that catch turns one bad line into "(skipped — rate-limited…)" for every line after it. And **a tier that translated NO line throws its last failure** (ruling E3-g, E3.S7), whatever kind it is: a list of nothing but placeholders reads to `ChainTranslator` as a success and would leave a healthy tier untried. Placeholders are for a PARTIAL failure only.
