...
- The gtx per-line fallback latches on a REFUSAL only (`RateLimited`/`Blocked`, E3.S6): a timeout or an unparseable body fails its own line and the loop keeps going. Widening that catch again turns one bad line into "(skipped — rate-limited…)" for every line after it.
