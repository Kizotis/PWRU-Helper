...
| E8.S2 `BergamotTranslator` | `aa6c00c` | (review) | → 1237 | `IBergamotEngine` seam (3 calls; fake in tests, suite never loads the native DLL); lazy `Load`/`Unload`/`IsLoaded`, never-pad batches, model missing → typed `Unavailable` + gate window → chain skips (E8-c, no `NotSent`); managed binding referenced with `ExcludeAssets="native"`, `bergamot.dll` absent from `bin/` (pinned); IS-10 strengthened. |
