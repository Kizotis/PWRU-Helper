...
Size: **measured** at 2000 entries × **502 B ≈ 981 KB** (U8 / E4.S3, 2026-09-07 —
`docs/investigations/03-stories/spikes/U8-cache-load.md`). The ~150 B estimate this line used to carry counted
Cyrillic at its two UTF-8 bytes; `JsonSerializer`'s default encoder escapes it to `\uXXXX`, six bytes a character.
The load costs **17.8 ms** (median, warm) on the calling thread of the first miss for **≈1 MB** of heap — inside
