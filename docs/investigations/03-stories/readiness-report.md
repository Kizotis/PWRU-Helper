...
| FR34 | Key test without spending quota | E6.S5 | ⚠ conditional — see **R-1** |
...
| OQ-6 | Can a key be validated without spending quota? | **Unanswered.** The architecture never specifies a validation endpoint | ⚠ **R-1** |
...
|---|---|---|---|---|---|
| **R-1** | Gap | **OQ-6 unanswered:** can a key be validated without spending quota? Sally's `Test key` copy (§3.7) implies a free check; the architecture specifies no validation endpoint. | Use a non-billing endpoint where one exists (DeepL `/usage`, Azure `/languages` or equivalent). Where none exists, the button label becomes `Test key (uses a few characters)` — the copy must not claim a free check that is not free. Already written into E6.S5's ACs. | **W** (design), **S** (label) | E6.S5 |
