...
public class CacheLoadSpike
{
    ...
    /// two bytes a character in UTF-8 and <b>six</b> once <c>JsonSerializer</c>'s default encoder
    /// escapes it, which is most of what this spike is here to find out. Deterministic seed so two
    /// runs on two machines compare.</summary>
    private static List<(string Key, string Value, string Provider)> Entries(int n)
    {
