...
public class CountdownTests
{
    ...
    /// time format is the culture's <c>TimeSeparator</c> and its digits are the culture's, so a
    /// machine whose locale spells numbers differently would render a countdown the player reads as
    /// something else. The culture here is deliberately hostile — one that happened to agree with
    /// the invariant one would make this test pass while proving nothing.
    /// </summary>
    [Fact]
    ...
    /// nothing, so no timer exists before the first paint — and none can, because nothing issues a
    /// request before it (<c>ProviderGates</c> loads <c>provider-state.json</c> on the first
    /// <c>TryEnter</c>, not at construction).</para>
    /// </summary>
    [Fact]
    ...
    public void The_timer_is_stopped_by_the_window_and_by_stopping_live()
    {
        ...
        var live = File.ReadAllText(RepoFile("MainWindow.Live.cs"));

        int closing = main.IndexOf("protected override void OnClosing", StringComparison.Ordinal);
        Assert.True(closing >= 0);
        Assert.Contains("StopCountdown();", main[closing..], StringComparison.Ordinal);

        int stopLive = live.IndexOf("private void StopLive()", StringComparison.Ordinal);
        Assert.True(stopLive >= 0, "StopLive must still be where the countdown is turned off");
        Assert.Contains("StopCountdown();", live[stopLive..], StringComparison.Ordinal);
    }
